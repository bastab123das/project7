...
# Create tables for students, courses, and enrollments if they do not exist
cursor.execute('''
...
# Allow the user to select a course when registering a student
if not courses.empty:
    course_options = courses.apply(lambda x: f"{x['id']}: {x['name']} ({x['duration']})", axis=1)
    selected_course = st.selectbox("Select Course", course_options)
    course_id = int(selected_course.split(":")[0])  # Extract the course_id from the selected option
else:
    course_id = None
    st.warning("Please add courses before registering students.")

if st.button("Add Student"):
    if name and roll_no and department:
        try:
            cursor.execute("INSERT INTO students (name, roll_no, department, course_id) VALUES (?, ?, ?, ?)",
                           (name, roll_no, department, course_id))
            conn.commit()
...
# Step 4: Enroll a student in a course (this remains the same as before)
...
if view_option == "Students":
    # Join students with courses to display course names along with student information
    students_df = pd.read_sql_query("""
        SELECT students.id, students.name, students.roll_no, students.department, courses.name AS course_name
        FROM students
        LEFT JOIN courses ON students.course_id = courses.id
    """, conn)
    st.dataframe(students_df)
elif view_option == "Courses":
    courses_df = pd.read_sql_query("SELECT id, name, duration FROM courses", conn)
